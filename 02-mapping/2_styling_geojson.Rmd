...
```{r multi_style}
spp <- c('Accipiter gentilis','Accipiter striatus','Accipiter cooperii')
dat <- occ(query = spp, from = c("gbif","bison"), gbifopts = list(georeferenced = TRUE))
dat <- fixnames(dat)
dat <- occ2df(dat)

map_dat <- stylegeojson(input = dat, var_col = "name", color = c("#8BA8D9", "#8BD99D", "#FFEF0D"), var_sym = "prov", symbol=c("g","b"))
mapgist(map_dat)
